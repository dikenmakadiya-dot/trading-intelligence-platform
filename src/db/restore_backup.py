...
def list_backups(backup_dir: Optional[Union[str, Path]] = None) -> List[Dict[str, Any]]:
    ...
    print("\n" + "=" * 80)
    print("?? QUANTFLOW TRADING PLATFORM - BACKUP SNAPSHOT CATALOG")
...
def restore_from_snapshot(
    ...
    # 1. Create safety backup of current target DB if it exists
    if target_db_path.exists() and create_safety_backup:
    ...
    if snap_db_file.exists() and snap_db_file.stat().st_size > 0:
        ...
        for tbl in tables:
            ...
            if df is not None and not df.empty:
                # Write to database
                df.to_sql(tbl, db.conn, if_exists="replace", index=False)
